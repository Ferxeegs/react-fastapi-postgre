...
export default function HppEntityFactorsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const [form, setForm] = useState<{ entity_type: string; category: string; percentage: string }>({
    entity_type: "",
    category: "",
    percentage: "",
  });
  ...
  const resetForm = () => {
    setForm({ entity_type: "", category: "", percentage: "" });
    setEditingId(null);
  };
  const filteredRows = rows.filter((item) =>
    `${item.entity_type} ${item.category}`.toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-2 sm:gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1 max-w-md">
            <input
              type="text"
              placeholder="Cari entity/category..."
              value={search}
          ...
          <div className="overflow-x-auto">
            <Table className="w-full text-left border-collapse">
              <TableHeader className="border-b border-gray-200 bg-gray-50/50 dark:border-gray-800 dark:bg-white/[0.02]">
                <TableRow>
                  {["No.", "Tipe Entitas", "Kategori", "Rate", "Aksi"].map((h) => (
              ...
              <TableBody className="divide-y divide-gray-200 dark:divide-gray-800">
                ...
                {filteredRows.map((r, index) => (
                  <TableRow key={r.id} className="transition-colors hover:bg-gray-50/80 dark:hover:bg-white/[0.01]">
                    ...
                    <TableCell className="px-6 py-4 text-[13px] text-gray-600 dark:text-gray-300">{r.entity_type}</TableCell>
                    <TableCell className="px-6 py-4 text-[13px] text-gray-600 dark:text-gray-300">{r.category}</TableCell>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setForm({
                                ...
                                category: r.category,
                                percentage:
                                  r.percentage == null || r.percentage === "" ? "" : String(Number(r.percentage)),
                              });
                              openModal();
                            }}
                          >
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[500px] p-6">
        <div>
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
            {editingId ? "Edit Faktor Entitas" : "Tambah Faktor Entitas"}
          </h3>
        </div>
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                const payload = { entity_type: form.entity_type, category: form.category, percentage };
                if (editingId) {
                  await hppAPI.updateAdminEntityFactor(editingId, payload);
                  success("Faktor entitas berhasil diperbarui!");
                } else {
                  await hppAPI.createAdminEntityFactor(payload);
