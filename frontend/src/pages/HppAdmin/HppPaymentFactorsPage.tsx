...
export default function HppPaymentFactorsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const filteredRows = rows.filter((item) =>
    `${item.lease_term} ${item.description || ""}`.toLowerCase().includes(search.toLowerCase())
  );

  return (
    <>
      {!embedded && <PageMeta title="Payment Adjustment Factors" description="Kelola faktor penyesuaian pembayaran" />}
      {!embedded && <PageBreadcrumb pageTitle="Faktor Penyesuaian Pembayaran" />}
      <div className="space-y-4">
        <div className="flex flex-col gap-2 sm:gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1 max-w-md">
            <input
              type="text"
              placeholder="Cari lease term/deskripsi..."
              value={search}
          ...
          <div className="overflow-x-auto">
            <Table className="w-full text-left border-collapse">
              <TableHeader className="border-b border-gray-200 bg-gray-50/50 dark:border-gray-800 dark:bg-white/[0.02]">
                <TableRow>
                  {["No.", "Durasi Sewa", "Rate", "Deskripsi", "Aksi"].map((h) => (
              ...
              <TableBody className="divide-y divide-gray-200 dark:divide-gray-800">
                ...
                {filteredRows.map((r, index) => (
                  <TableRow key={r.id} className="transition-colors hover:bg-gray-50/80 dark:hover:bg-white/[0.01]">
                    ...
                    <TableCell className="px-6 py-4 text-[13px] text-gray-600 dark:text-gray-300">{r.lease_term} Tahun</TableCell>
                    <TableCell className="px-6 py-4 text-[13px] font-medium text-gray-600 dark:text-gray-300">{r.rate}%</TableCell>
                    <TableCell className="px-6 py-4 text-[13px] text-gray-600 dark:text-gray-300 max-w-xs truncate" title={r.description}>{r.description}</TableCell>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[500px] p-6">
        <div>
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
            {editingId ? "Edit Faktor Pembayaran" : "Tambah Faktor Pembayaran"}
          </h3>
        </div>
        <div className="mt-6 flex flex-col gap-4">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Durasi Sewa (Tahun)</label>
            <input placeholder="Contoh: Bulanan" value={form.lease_term} onChange={(e) => setForm({ ...form, lease_term: e.target.value })} className="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:text-white dark:focus:border-brand-500" />
          </div>
          <div>
