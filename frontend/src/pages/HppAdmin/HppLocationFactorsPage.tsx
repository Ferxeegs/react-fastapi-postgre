          ...
          <div className="overflow-x-auto">
            <Table className="w-full text-left border-collapse">
              <TableHeader className="border-b border-gray-200 bg-gray-50/50 dark:border-gray-800 dark:bg-white/[0.02]">
                <TableRow>
                  {["No.", "Nama Lokasi", "Rate", "Aksi"].map((h) => (
              ...
              <TableBody className="divide-y divide-gray-200 dark:divide-gray-800">
                ...
                {filteredRows.map((r, index) => (
                  <TableRow key={r.id} className="transition-colors hover:bg-gray-50/80 dark:hover:bg-white/[0.01]">
                    ...
                    <TableCell className="px-6 py-4 text-[13px] text-gray-600 dark:text-gray-300">{r.location}</TableCell>
                    <TableCell className="px-6 py-4 text-[13px] font-medium text-gray-600 dark:text-gray-300">{r.percentage}%</TableCell>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[500px] p-6">
        <div>
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
            {editingId ? "Edit Faktor Lokasi" : "Tambah Faktor Lokasi"}
          </h3>
        </div>
        <div className="mt-6 flex flex-col gap-4">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Lokasi</label>
            <input placeholder="Contoh: Pusat Kota" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:text-white dark:focus:border-brand-500" />
          </div>
          <div>
